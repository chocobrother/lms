			...
			<div id="content">
				...
				<tr>
						<td colspan ="6" align = "center">
							...
							[<a id = "paging" href = "${pageContext.servletContext.contextPath }/admin?pg=${map.startPage -1}">이전</a>]	
							</c:if>	
						<c:forEach var="i" begin="${map.startPage }" end="${map.endPage }" step="1">
							<c:if test="${i eq map.pg }">
								[<a id="currentPaging" href="${pageContext.servletContext.contextPath }/admin?pg=${i}">${i}</a>]
							</c:if>
							
							<c:if test="${i ne map.pg }">
								[<a id="paging" href="${pageContext.servletContext.contextPath }/admin?pg=${i}">${i}</a>]
							</c:if>
							</c:forEach>
							<c:if test="${map.endPage < map.totalP }">
								[<a id = "paging"
								href="${pageContext.servletContext.contextPath }/admin?pg=${map.endPage+1 }">다음</a>]
							</c:if>
					</td>
				</tr> 
